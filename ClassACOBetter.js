...
ClassACO.prototype = {
  ...
  superRT:function(ant){
    ...
    if (!dests.length)return 0; 
    var sp=JSON.parse(JSON.stringify(_thisP[index]));
    sp.sort(function(a,b){
      return a.pheromone_val-b.pheromone_val
    })
    dests.forEach(function(val,ind){
      sp.forEach(function(v,i){
        if(v.dest==val){
          ...
          var alpha=1;
          v.pheromone_val=fb(i)
          t.pheromone_val=fb(i)+p_sum;
          dest_probs.push(t);//val=v;
          p_sum+=fb(i)
        }
      });
...
ClassACO.prototype.leavePHForAnts = function (Ants){
  // this.superRT({
  //   nowAt:"-JdRybjolbHukKuVxlMi",
  //   trail:["-JdRybjolbHukKuVxlMi"]
  // })
  var _this = this;
  var bestNumber = Math.floor(Ants.length*0.1)
  //console.debug(Ants)
  var ants = [] 
  for(var i=0;i<bestNumber;i++)ants.push(Ants[i])
  ants = ants.reverse();
  var fabb=[1,2]
  ants.forEach(function(ant,ind){
    ...
    if(ind<2)f=fabb[ind];
      ...
      f = fabb[ind-2]+fabb[ind-1]
      fabb.push(f);
    }
    _this.leavePh(ant,f);
  });
}
ClassACO.prototype.leavePh=function(ant,f){
  var p=ant.trail.slice(0);
  var p_ini=p.shift();
  var p_to;
  while (p_to=p.shift()){
    if(this.mode>0)
      this.addPh(p_ini,p_to,f);
    else 
      this.addPh(p_ini,p_to,f);
    p_ini=p_to;
  }
...
ClassACO.prototype.calEnergy=function(src,dest,pre){
  ...
  else if(theta<30)ret+=0.35   // straigt
